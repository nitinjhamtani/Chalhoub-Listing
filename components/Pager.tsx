...
const Pager: React.FC<{ totalItems: number }> = ({ totalItems }) => {
  ...
  const getDisbaled = useMemo(() => ((pageSize * (page+1)) > totalItems), [page, pageSize, totalItems]);
  ...
  <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
    <div>
      <p className="text-sm text-gray-700">
        Showing{" "}
        <span className="font-medium">{(pageSize * (page)) + 1}</span>{" "}
        to{" "}
        <span className="font-medium">
          {getDisbaled ? totalItems : pageSize * (page+1)}
        </span>{" "}
        of{" "}
        <span className="font-medium">
          {totalItems}
    ...
    <div>
      <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
        ...
        <button disabled={getDisbaled} onClick={(e) => setPage((prev: number) => prev + 1)} className="relative inline-flex items-center rounded-r-md border disabled:bg-gray-300 border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20">
